...
const MyFavorites = ({ recipes, currentUser }) => {
    console.log(recipes)
    ...
    return (
        <>
            ...
            <div className="recipe-list">
                {recipes.filter(recipe => currentUser.favorites.includes(recipe.id.toString())).filter(recipe => recipe.vegetarian === true || recipe.vegetarian === searchVegetarian).filter(recipe => recipe.ingredients.toLowerCase().includes(searchTerm.toLowerCase())).map((recipe, index) => (
                    <>
